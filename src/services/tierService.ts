...
export function canAccessTier(userTier: string, contentTier: string): boolean {
  const userLevel = TIER_LEVELS[isValidTier(userTier) ? (userTier as ValidTier) : 'free'];
  const contentLevel = TIER_LEVELS[isValidTier(contentTier) ? (contentTier as ValidTier) : 'free'];
 ...
 */
export function useUserTier(client: SupabaseClient | null, userId: string | null): UserTierResult {
  ...
  const fetchUserTier = useCallback(async () => {
    ...
    try {
      const { data, error } = await client.rpc('get_user_tier', { user_id_param: userId });
      ...
      if (isValidTier(data)) {
        ...
        console.warn(`Invalid tier value: ${data || 'none'}, defaulting to 'free'`);
        setUserTier('free');
      }
    } catch (err: any) {
