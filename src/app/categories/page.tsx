...
function CategoriesContent() {
  ...
  const categoryCounts = useMemo(() => {
    ...
    projects.forEach((project: Project) => {
      if (!project.deleted_at) { // Don't count deleted projects
            ...
            {Object.entries(PROJECT_CATEGORIES).map(([key, category]) => {
              ...
              return (
                <Card 
                  ...
                  onClick={() => hasProjects && router.push(`/categories/${encodeURIComponent(key)}`)}
                >
                  <CardHeader className="flex flex-row justify-between items-center">
                    <CardTitle>{category.label}</CardTitle>
                    <Badge 
                      variant={hasProjects ? "secondary" : "outline"} 
                      className={`category-count ml-2 ${!hasProjects ? 'opacity-50' : ''}`}
                    >
                  ...
                  <CardContent className="space-y-4">
                    ...
                    >
                      {hasProjects ? `Browse ${category.label}` : 'No projects yet'}
                    </Button>
                  </CardContent>
                </Card>
              );
